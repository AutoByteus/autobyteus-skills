...
`Product Manager -> Product Feature Brief -> Engineering Intake / Stage 0 -> Stage 1-10 Engineering Delivery -> Delivery/Deployment Engineer -> Product Manager Acceptance Packet -> Product Manager Acceptance -> Next Feature Brief`
...
- Product-iteration verification override: in product-iteration mode, Stage 10 must not ask the user for routine verification. It sends a Product Manager acceptance packet instead; Product Manager acceptance is the product-loop verification signal. User verification is only required if Product Manager blocks on a user/product decision, an external side effect requires explicit user approval, or the user explicitly requested manual verification.
...
- Product Manager owns product opportunity selection, priority rationale, initial feature proposal for PM-first runs, product acceptance in product-iteration runs, and next-feature proposal after accepted delivery completion.
...
- If `send_message_to` and recipient `product_manager` are available, Stage 10 sends a self-contained acceptance request to `product_manager` with relevant artifact paths. If messaging or the recipient is unavailable, Stage 10 persists the packet/status and records `Pending` or `Blocked`; do not mark the Product Manager acceptance callback as `Sent` unless the send succeeds.
- The Product Manager loop cannot bypass Stage 0-10 gates, code-edit locks, Stage 7 validation, Stage 8 review, Stage 9 docs sync, Product Manager acceptance, ticket archival, repository finalization, release/publication/deployment rules, or cleanup. Normal one-off runs still require explicit user verification.
...
- Do not close the task until post-testing `docs/` synchronization is completed (or explicit no-impact decision is recorded with rationale), and do not mark final completion until required Stage 10 verification/archive/finalization work is complete and, when product-iteration mode is active, Product Manager acceptance callback status is recorded truthfully.
...
- Keep the ticket folder under `tickets/in-progress/` until the applicable verification path is satisfied: explicit user completion confirmation for one-off runs or Product Manager acceptance / acceptance-handoff status for product-iteration runs.
...
| 10 | Final Handoff | Delivery summary ready + applicable verification signal (explicit user verification for one-off runs, Product Manager acceptance for product-iteration runs) -> move ticket to `done` when finalization is in scope -> repository finalization into resolved target branch (when git repo) + any applicable release/publication/deployment step completed or explicitly recorded as not required + required post-finalization worktree/branch cleanup complete when applicable + ticket state decision recorded + Product Manager acceptance callback status recorded when product-iteration mode is active | Locked |
...
| 10 Final Handoff | Handoff summary is complete, the applicable verification signal is received or handed off (`User Verified` for one-off runs; Product Manager acceptance request sent/persisted and acceptance status tracked for product-iteration runs), the ticket has been moved to `tickets/done/<ticket-name>/` when finalization is in scope, and, when in a git repository, repository finalization is complete, any applicable release/publication/deployment step is complete or explicitly recorded as not required, required post-finalization worktree/branch cleanup is complete when applicable, and Product Manager acceptance callback status is recorded when product-iteration mode is active | Stay in `Stage 10` until the applicable verification/acceptance path is satisfied and Stage 10 archival/finalization/cleanup is complete; if PM messaging is unavailable, persist `Pending`/`Blocked` status instead of claiming success | End |
...
| Stage 10 awaiting verification/acceptance | stay in `Stage 10 (In Progress)` | For one-off runs, wait for explicit user completion/verification. For product-iteration runs, send/persist Product Manager acceptance packet and track acceptance status instead of waiting on the user. |
...
- Handoff summary must include:
  ...
  - Product Manager acceptance callback status when product-iteration mode is active (`Sent` / `Pending` / `Blocked`) plus recipient, packet source/path, sent timestamp or pending/blocker reason, acceptance status, and next-iteration status.
...
- After the handoff summary is written, keep Stage 10 open until the applicable verification path is satisfied. For normal one-off runs, wait for the user to explicitly confirm completion/verification. For product-iteration runs, send the Product Manager acceptance packet instead of asking the user for routine verification. Before the applicable verification signal, do not move the ticket to `done`, commit, push, merge, or run release/publication/deployment work.
...
- When product-iteration mode is active, after the handoff summary and delivery evidence are truthful enough for product acceptance, prepare the Product Manager acceptance packet from the handoff summary and send it with `send_message_to(product_manager)` when that tool and recipient are available. The message content must be self-contained and include reference artifact paths. If the send is unavailable, persist the packet path/status in `handoff-summary.md` and `workflow-state.md` as `Pending` or `Blocked`; do not mark the callback as `Sent`.
- Product Manager acceptance packet fields must include: ticket name, delivered scope, source Product Feature Brief / requirements reference when available, verification summary, docs sync result, release/publication/deployment/finalization state or explicit not-yet-finalized status, residual risks or deferred items, relevant artifact paths, product implications or follow-up context, and a request for Product Manager to accept or route rework and, if accepted, propose the next feature.
- Delivery/Deployment Engineer must not decide the next feature or self-accept product-loop work. Acceptance and the next feature must come from Product Manager and route back through Engineering Intake / Stage 0.
...
- Stage 10 (final handoff + verification/acceptance + move ticket to done + repository finalization + cleanup):
  ...
  - persist the handoff summary and wait for applicable verification/acceptance signal
  ...
  - on applicable verification/acceptance signal, first move the ticket folder to `tickets/done/<ticket-name>/`
  ...
  - when product-iteration mode is active, send `send_message_to(product_manager)` with a self-contained acceptance packet and relevant artifact paths when available; otherwise persist the packet path and mark callback status `Pending` or `Blocked` with reason
  - keep ticket in `tickets/in-progress/<ticket-name>/` unless the applicable verification/acceptance path is satisfied or the user explicitly asks to move it
