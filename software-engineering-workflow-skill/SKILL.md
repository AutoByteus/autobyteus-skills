...
- Product Manager owns product opportunity selection, priority rationale, initial feature proposal for PM-first runs, and next-feature proposal after delivery completion.
...
- Stage 10 Product Manager callback rule (mandatory when product-iteration mode is active, including runs that started in the Product Iteration Team): after explicit user verification and all required Stage 10 archival/finalization/release-publication-deployment/cleanup work are complete enough to report truthfully, send the completion packet to `product_manager` with `send_message_to` when available. If messaging or the recipient is unavailable, persist the packet path and record the callback as `Pending` or `Blocked`; do not claim callback success.
