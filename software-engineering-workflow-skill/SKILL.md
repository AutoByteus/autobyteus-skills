...
Run a staged software-engineering delivery workflow for software changes: bootstrap ticket context, investigate and refine requirements, build design and future-state runtime artifacts, drive implementation with one implementation artifact that carries both a stable baseline and live progress tracking, validate behavior with API/E2E and other executable validation evidence appropriate to the system, apply independent code review, synchronize long-lived docs, and finish with a verified handoff and repository finalization. Normal one-off runs use explicit user verification. Product-iteration runs use Product Manager acceptance as the Stage 10 product verification signal so the loop can continue without waiting for the user after each feature. For product-iteration mode, a dedicated Product Manager owns the outer loop that proposes the next feature, accepts or rejects completed product-loop deliveries, receives the Delivery/Deployment Engineer acceptance packet after Stage 10 evidence is truthful, and proposes the next feature without bypassing the engineering gates. For medium/large scope, include a full proposed design document organized by data-flow spine inventory, ownership, off-spine concerns, and derived separation of concerns.
...
`Product Manager -> Product Feature Brief -> Engineering Intake / Stage 0 -> Stage 1-10 Engineering Delivery -> Delivery/Deployment Engineer -> Product Manager Acceptance Packet -> Product Manager Acceptance -> Product Iteration Plan Update -> Next Feature Brief`

- In the AutoByteus runtime, the normal `Software Engineering Team` starts with `solution_designer` and product iteration is inactive by default. The separate `Product Iteration Team` starts with `product_manager` and product iteration is active by default.
- Product-iteration verification override: in product-iteration mode, Stage 10 must not ask the user for routine verification. It sends a Product Manager acceptance packet instead; Product Manager Acceptance Status = `Accepted` is the product-loop verification signal. User verification is only required if Product Manager blocks on a user/product decision, an external side effect requires explicit user approval, or the user explicitly requested manual verification.
- The canonical Product Manager is the team-local `product_manager` agent in `autobyteus-agents`; this skill documents the workflow behavior and does not define the canonical agent package.
- Product Manager owns product opportunity selection, Product Iteration Plan/backlog/cursor, priority rationale, exactly one initial feature proposal for PM-first runs, product acceptance in product-iteration runs, and exactly one next-feature proposal after accepted delivery acceptance.
- Product Manager does not own requirements refinement, architecture, implementation, technical validation ownership, code review, docs sync, repository finalization, release/publication/deployment, or source-code edits.
...
- Engineering Intake / Stage 0 accepts a Product Manager feature brief as input evidence, whether it came from the Product Iteration Team's initial PM-first intake or from a post-delivery next-feature callback, then still creates the ticket/worktree/bootstrap artifacts and `requirements.md` through the normal Stage 0 controls.
- Delivery/Deployment Engineer (the existing `deployment_engineer` Stage 10 role) owns the Product Manager acceptance packet after Stage 10 delivery evidence is truthful. Product Manager owns product acceptance and the next-feature decision after receiving that packet.
- If `send_message_to` and recipient `product_manager` are available, Stage 10 sends a self-contained acceptance packet to `product_manager` with relevant artifact paths. If messaging or the recipient is unavailable, Stage 10 persists the packet/status and records Acceptance Callback Status as `Pending` or `Blocked`; do not mark the acceptance callback as `Sent` unless the send succeeds. Callback `Sent` is not Product Manager `Accepted`.
- The Product Manager loop cannot bypass Stage 0-10 gates, code-edit locks, Stage 7 validation, Stage 8 review, Stage 9 docs sync, Product Manager acceptance, ticket archival, repository finalization, release/publication/deployment rules, or cleanup. Product Iteration Loop Status remains separate from per-ticket completion; accepted tickets can finalize while the outer loop remains `Active`. Normal one-off runs still require explicit user verification.
...
- Move rule (mandatory): move a ticket from `in-progress` to `done` only when the applicable verification signal is satisfied. For one-off runs, that signal is explicit user completion/verification (for example: "done", "finished", or "verified") or an explicit user move instruction. For product-iteration runs, that signal is Product Manager Acceptance Status = `Accepted`; Acceptance Callback Status `Sent` / `Pending` / `Blocked` alone is not sufficient.
- Final archive ordering rule (mandatory): when the applicable verification signal also triggers repository finalization, move the ticket folder to `tickets/done/<ticket-name>/` before the final commit so the committed state includes the archived ticket path.
...
- Do not close the task until post-testing `docs/` synchronization is completed (or explicit no-impact decision is recorded with rationale), and do not mark final completion until required Stage 10 verification/archive/finalization work is complete and, when product-iteration mode is active, Acceptance Callback Status is recorded truthfully and separately from Product Manager Acceptance Status.
- Verification hold rule (mandatory): after Stage 9 passes, persist the handoff summary and keep Stage 10 open until the applicable verification owner has acted. For normal one-off runs, the owner is the user and an explicit user completion/verification signal is required. For product-iteration runs, the owner is `product_manager`; send the acceptance packet instead of asking the user for routine verification. Do not commit, push, merge, run release/publication/deployment work, or move the ticket to `done` before the applicable verification signal unless the project explicitly documents a safe pre-verification checkpoint.
...
- Stage 10 Product Manager acceptance callback rule (mandatory when product-iteration mode is active, including runs that started in the Product Iteration Team): once delivery evidence, docs sync, and handoff status are truthful enough for product acceptance, send the acceptance packet to `product_manager` with `send_message_to` when available instead of waiting for routine user verification. If messaging or the recipient is unavailable, persist the packet path and record Acceptance Callback Status as `Pending` or `Blocked`; do not claim callback success. Product Manager accepts or routes rework, and if accepted updates the Product Iteration Plan and proposes the next feature.
- Stage 10 blockage rule (mandatory): if the move to `tickets/done/`, commit, push, target-branch update, merge, or required post-finalization cleanup fails after the applicable verification signal, keep Stage 10 `In Progress`/`Blocked`, record the blocker in `workflow-state.md`, and do not mark final handoff complete. If a release/publication/deployment step is applicable and later fails or is undocumented, record that blocker too, but do not undo completed repository finalization.
- Keep the ticket folder under `tickets/in-progress/` until the applicable verification path is satisfied: explicit user completion confirmation for one-off runs or Product Manager Acceptance Status = `Accepted` for product-iteration runs. Callback `Sent` or persisted packet status alone is not sufficient.
- Treat engineering completion, verification/acceptance, ticket archival, repository finalization, release/publication/deployment, and post-finalization cleanup as separate gates: engineering completion ends at implementation + Stage 7 executable-validation gate + code review + docs sync; after that, use the applicable verification path; only then move the ticket to `tickets/done/` when finalization is in scope, complete required Stage 10 repository finalization, run any applicable release/publication/deployment work, and complete any required worktree/branch cleanup.
...
| 6 | Source Implementation + Unit/Integration | Source code + required unit/integration checks complete + no backward-compat/legacy retention + ownership-driven dependency quality preserved + Authoritative Boundary Rule preserved + touched files correctly placed | Unlocked |
...
| 10 | Final Handoff | Delivery summary ready + applicable verification signal (explicit user verification for one-off runs, Product Manager Acceptance Status = `Accepted` for product-iteration runs) -> move ticket to `done` when finalization is in scope -> repository finalization into resolved target branch (when git repo) + any applicable release/publication/deployment step completed or explicitly recorded as not required + required post-finalization worktree/branch cleanup complete when applicable + ticket state decision recorded + Acceptance Callback Status recorded when product-iteration mode is active | Locked |
...
| 6 Source + Unit/Integration | Source implementation complete, required unit/integration checks pass, no backward-compatibility/legacy-retention paths remain in scope, ownership-driven dependencies remain valid (no new unjustified cycles/tight coupling), the Authoritative Boundary Rule remains valid (no new bypass of authoritative boundaries), and touched files sit in the correct folder under the correct owning subsystem | Local issues: stay in `6`; classified re-entry for non-local issues (`Design Impact`: `1 -> 3 -> 4 -> 5 -> 6`, `Requirement Gap`: `2 -> 3 -> 4 -> 5 -> 6`, `Unclear`: `0 -> 1 -> 2 -> 3 -> 4 -> 5 -> 6`) | `7` |
...
| 10 Final Handoff | Handoff summary is complete, the applicable verification signal is satisfied (`User Verified` for one-off runs; Product Manager Acceptance Status = `Accepted` for product-iteration runs), the ticket has been moved to `tickets/done/<ticket-name>/` when finalization is in scope, and, when in a git repository, repository finalization is complete, any applicable release/publication/deployment step is complete or explicitly recorded as not required, required post-finalization worktree/branch cleanup is complete when applicable, and Acceptance Callback Status is recorded separately when product-iteration mode is active | Stay in `Stage 10` until the applicable verification/acceptance path is satisfied and Stage 10 archival/finalization/cleanup is complete; if PM messaging is unavailable, persist `Pending`/`Blocked` status instead of claiming success | End |
...
| Stage 10 awaiting verification/acceptance | stay in `Stage 10 (In Progress)` | For one-off runs, wait for explicit user completion/verification. For product-iteration runs, send/persist the Product Manager acceptance packet, record Acceptance Callback Status, and wait for Product Manager Acceptance Status = `Accepted` before archival/finalization. |
...
| Stage 10 Product Manager acceptance callback unavailable in product-iteration mode | stay in `Stage 10` until the acceptance packet path/status is persisted | Record `Pending` or `Blocked` with recipient `product_manager`, packet evidence, and reason; only a successful `send_message_to(product_manager)` is `Sent`. |
...
- `Authoritative Boundary Rule` (mandatory): when one boundary is the authoritative public entrypoint for a domain subject, callers above it should depend on that boundary rather than on both the boundary and one of its internal owned mechanisms at the same time. This is the `no boundary bypass / no mixed-level dependency` rule.
...
- Review each use case against these criteria:
  ...
  - Authoritative Boundary Rule check (`Pass`/`Fail`): callers use the authoritative boundary instead of depending on both an outer owner and one of its internal lower-level concerns,
...
- Score-row ownership note (mandatory): ownership-driven dependency quality and shortcut avoidance are judged mainly inside `Ownership Clarity and Boundary Encapsulation` plus `API / Interface / Query / Command Clarity` and `Separation of Concerns and File Placement`, rather than as a separate top-level score row. The highest-signal check inside that area is the `Authoritative Boundary Rule`.
- Mandatory review checks:
  ...
  - Authoritative Boundary Rule preservation (no caller depends on both an authoritative boundary and one of its internal lower-level concerns),
...
- Handoff summary must include:
  ...
  - verification summary (unit/integration plus Stage 7 executable validation, acceptance-criteria closure status, and for infeasible criteria documented constraints + compensating automated evidence + explicit user waiver or Product Manager acceptance-waiver reference as applicable),
  ...
  - Acceptance Callback Status when product-iteration mode is active (`Sent` / `Pending` / `Blocked`) plus recipient, packet source/path, sent timestamp or pending/blocker reason, Product Manager Acceptance Status, Product Iteration Loop Status, and next-iteration status.
- When release notes are required, create/update `tickets/in-progress/<ticket-name>/release-notes.md` before waiting for the applicable verification signal.
...
- After the handoff summary is written, keep Stage 10 open until the applicable verification path is satisfied. For normal one-off runs, wait for the user to explicitly confirm completion/verification. For product-iteration runs, send the Product Manager acceptance packet, record Acceptance Callback Status, and wait for Product Manager Acceptance Status = `Accepted` instead of asking the user for routine verification. Before the applicable verification signal, do not move the ticket to `done`, commit, push, merge, or run release/publication/deployment work.
- After the applicable verification signal, move the ticket folder to `tickets/done/<ticket-name>/` first so the archived ticket path is included in the final committed state when repository finalization is in scope.
...
- When product-iteration mode is active, after the handoff summary and delivery evidence are truthful enough for product acceptance, prepare the Product Manager acceptance packet from the handoff summary and send it with `send_message_to(product_manager)` when that tool and recipient are available. The message content must be self-contained and include reference artifact paths. If the send is unavailable, persist the packet path/status in `handoff-summary.md` and `workflow-state.md` as `Pending` or `Blocked`; do not mark Acceptance Callback Status as `Sent`. Callback `Sent` is not Product Manager Acceptance Status = `Accepted`.
- Product Manager acceptance packet fields must include: ticket name, delivered scope, source Product Feature Brief / requirements reference when available, verification summary, docs sync result, release/publication/deployment/finalization state or explicit not-yet-finalized status, residual risks or deferred items, relevant artifact paths, product implications or follow-up context, Product Iteration Loop Status if known, and a request for Product Manager to accept or route rework and, if accepted, update the Product Iteration Plan and propose the next feature.
- Delivery/Deployment Engineer must not decide the next feature or self-accept product-loop work. Acceptance, Product Iteration Plan updates, and the next feature must come from Product Manager and route back through Engineering Intake / Stage 0.
- Ticket state transition:
  - keep ticket under `tickets/in-progress/<ticket-name>/` by default after handoff and while waiting for applicable verification/acceptance,
  - on the applicable verification signal (explicit user confirmation that the ticket is finished/verified for one-off runs, or Product Manager Acceptance Status = `Accepted` for product-iteration runs), first move the ticket to `tickets/done/<ticket-name>/`, then run repository finalization if applicable.
...
- Stage 10 (final handoff + verification/acceptance + move ticket to done + repository finalization + cleanup):
  ...
  - Product-iteration Stage 10 sequence: handoff/release notes -> send or persist Product Manager acceptance packet -> record Acceptance Callback Status -> wait for Product Manager Acceptance Status = `Accepted` or route `Needs Rework` / `Blocked` -> archive/finalize
  - when product-iteration mode is active, send or persist the Product Manager acceptance packet before waiting for Product Manager acceptance: use `send_message_to(product_manager)` with a self-contained acceptance packet and relevant artifact paths when available; otherwise persist the packet path and mark Acceptance Callback Status `Pending` or `Blocked` with reason
  - when product-iteration mode is active, record Acceptance Callback Status (`Sent` / `Pending` / `Blocked`) and Product Manager Acceptance Status (`Requested` until Product Manager responds, then `Accepted` / `Needs Rework` / `Blocked`) in `workflow-state.md` and `handoff-summary.md`
  - when product-iteration mode is active, wait for Product Manager Acceptance Status = `Accepted` before archival/finalization; Product Manager Acceptance Status `Needs Rework` or `Blocked` routes the stated issue instead of finalizing, and callback `Sent` / `Pending` / `Blocked` alone is not Product Manager `Accepted`
  - for one-off runs, persist the handoff summary and wait for explicit user completion/verification or explicit user move instruction
  - only after the applicable verification signal (`User Verified` for one-off runs; Product Manager Acceptance Status = `Accepted` for product-iteration runs), first move the ticket folder to `tickets/done/<ticket-name>/`
  - if git repo and applicable verification/acceptance is complete, commit all in-scope changes on the ticket branch/worktree, including the moved ticket files
  - if git repo and applicable verification/acceptance is complete, push the ticket branch to remote
  - if git repo and applicable verification/acceptance is complete, update the resolved finalization target branch from remote, merge the ticket branch into it, and push the updated target branch
  - if git repo and applicable verification/acceptance is complete and the project has an applicable documented release/publication/deployment step, run that method after repository finalization and feed it the archived ticket release-notes artifact (typically `tickets/done/<ticket-name>/release-notes.md`) when release notes are required
  ...
  - keep ticket in `tickets/in-progress/<ticket-name>/` unless the applicable verification/acceptance path is satisfied
