...
7. Create or update `tickets/in-progress/<ticket-name>/requirements.md` with status `Draft` from user input, Product Manager feature brief, Product Iteration Plan reference, selected slice ID, or bug evidence.
...
- When the ticket starts from a Product Manager feature brief or from the Product Iteration Team, record that source, Product Iteration Plan reference, selected slice ID, and Product Iteration Loop Status in `workflow-state.md` and preserve Product Manager as the product-loop owner and product acceptance owner; Stage 0 owns ticket/worktree/bootstrap and draft requirements, not product prioritization.
...
- if started from a Product Manager feature brief: brief source/path or message reference, Product Iteration Plan reference, and selected slice ID recorded
